...
{ 
  ...
  TString canvName = "MVA_etamatch_efficiency";
...

  {
    ...
    TH1F *data = static_cast<TH1F*>(file_.Get("DQMData/Run 1/MTD/Run summary/Vertices/MVAEtaMatchEff")->Clone());
    data->SetDirectory(0);
    data->SetMarkerStyle(20);
    data->SetMarkerSize(markerSize);

    data->GetXaxis()->SetTitleOffset(0);
    data->GetXaxis()->SetNdivisions(6,5,0);
    data->GetXaxis()->SetTitle("|#eta|");  
    data->GetYaxis()->SetNdivisions(6,5,0);
    data->GetYaxis()->SetTitle("Efficiency");  

    float t = canv->GetTopMargin();

    data->GetXaxis()->SetTitleSize(cmsTextSize*t);
    data->GetYaxis()->SetTitleSize(cmsTextSize*t);
